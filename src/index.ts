...
async function pipenvConvert(cmd: string, srcDir: string) {
  ...
  try {
    const { stdout: out } = await execa(cmd, [], {
      cwd: srcDir,
    });
...
export const build = async ({
  ...
  await runUserScript(workPath, meta);
...
function getUserScriptPath(workPath: string, scriptName = 'build'): string | null {
  if (process.platform === 'win32') {
    // looking for *.cmd
    const scriptPath = join(workPath, `${scriptName}.cmd`);
    return existsSync(scriptPath) ? scriptPath : null;
  }

  const omitted = join(workPath, `${scriptName}`);
  if (existsSync(omitted)) {
    return omitted;
  }

  const sh = join(workPath, `${scriptName}.sh`);
  if (existsSync(sh)) {
    return sh;
  }
...
async function runUserScript(workPath: string, meta: Meta, scriptName = 'build') {
  const scriptPath = getUserScriptPath(workPath, scriptName);
  if (scriptPath) {
    debug('Found user script', scriptPath);
    const ok = await runShellScript(scriptPath, [], { env: { ...process.env, ...meta.env } });
    debug('Run user script', scriptPath, ok);
    return ok;
  }
  return false;
}